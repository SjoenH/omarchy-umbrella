...
TestCase {
    ...
    function test_soon_counts_current_hour() {
        var s = series({12: 0.5})
        var rain = Model.nextRain(s[0], s[1], now)
        compare(rain.state, "soon")
    ...
    function test_barLabel_variants() {
        compare(Model.barLabel({state: "soon", minutesUntil: 45}), "☔ 45m")
        compare(Model.barLabel({state: "soon", minutesUntil: -1}), "☔")
        compare(Model.barLabel({state: "later", minutesUntil: -1}), "🌂")
        compare(Model.barLabel(null), "")
    }

    function test_rainWindows_contiguous_run() {
        var s = series({15: 0.3, 16: 0.2, 17: 0.4})
    ...
    function test_rainWindow_open_at_horizon() {
        ...
        compare(windows[0].end.getTime() - windows[0].start.getTime(), 3600000)
    }
    ...
    function test_locationCommit() {
        var choices = [{name: "Oslo", latitude: 59.9, longitude: 10.7}]
